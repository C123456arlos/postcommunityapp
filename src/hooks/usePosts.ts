...
const usePosts = () => {
  ...
  const onDeletePost = async (post: Post): Promise<boolean> => {
    try {
      ...
      setPostStateValue(prev => ({
        ...prev, posts: prev.posts.filter(item => item.id !== post.id)
      }))
      return true
    } catch (error) {
      return false
  ...
  const getCommunityPostVotes = async (communityId: string) => {
    ...
    setPostStateValue(prev => ({
      ...prev, postVotes: postVotes as PostVote[]
    }))
  }
  useEffect(() => {
  ...
  return {
    postStateValue, setPostStateValue, onVote, onSelectPost, onDeletePost
  }
}
...
//   }
//   return {
//     postStateValue, setPostStateValue, onVote, onSelect, onDelete
//   }
// }
